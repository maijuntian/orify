...
import 'package:gsy_github_app_flutter/test/demo_qrcode.dart';
import 'package:gsy_github_app_flutter/test/demo_webview.dart';
...
class FlutterReduxApp extends StatelessWidget {
  /// 创建Store，引用 MaiState 中的 appReducer 实现 Reducer 方法
  /// initialState 初始化 State
  final store = new Store<MaiState>(
    mAppReducer,
    middleware: mMiddleware,

    ///初始化数据
    initialState: new MaiState(
        userInfo: MUserEntity.empty(),
        locale: Locale('zh', 'CH')),
  );
  ...
  Widget build(BuildContext context) {
    ...
    return new StoreProvider(
      store: store,
      child: new StoreBuilder<MaiState>(builder: (context, store) {
            ...
            routes: {
              WelcomePage.sName: (context) {
                ...
                return new GSYLocalizations(
                    child: NavigatorUtils.pageContainer(new WelcomePage()));
              },
...
class _GSYLocalizations extends State<GSYLocalizations> {
  ...
  errorHandleFunction(int code, message) {
    print("event bus--> code:${code}  message:${message}");
    switch (code) {
